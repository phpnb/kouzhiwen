...
class Route
{
     ...
     * @param string    $type 绑定类型 默认为module 支持 namespace class common
     * @return mixed
     */
    ...
    public static function bindToController($url, $controller, $depr = '/')
    {
        ...
        return ['type' => 'common', 'common' => $controller . '/' . $action];
    }
    ...
    private static function parseRule($rule, $route, $pathinfo, $option = [], $matches = [])
    {
        ...
        if ($route instanceof \Closure) {
            ...
            $result            = ['type' => 'common', 'common' => implode('/', $route), 'var' => $var];
...
//        $has_child = config('has_child_module');
//        if (in_array($path_info[0], $has_child)) {
//            unset($path_info[0],$path_info[1],$path_info[2],$path_info[3]);
//            $url = str_replace('.html', '', implode('|', $path_info));
//        } 
        if(array_key_exists($_SERVER['HTTP_HOST'],config('has_child_module_url'))){
            unset($path_info[0],$path_info[1],$path_info[2]);
            $url = str_replace('.html', '', implode('|', $path_info));
        }
