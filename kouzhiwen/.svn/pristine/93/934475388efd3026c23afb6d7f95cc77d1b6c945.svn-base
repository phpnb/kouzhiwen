    ...
    public function pathinfo()
    {
        if (is_null($this->pathinfo)) {
            if (isset($_GET[Config::get('var_pathinfo')])) {
                ...
                // CLI模式下 index.php module/common/action/params/...
...
//            $this -> module = $path_info[0] . '\\' . $path_info[1];
//        }

        $has_child=config('has_child_module_url');

        if(array_key_exists($_SERVER['HTTP_HOST'], $has_child)){
            $module_module=config('default_module_module');
            $module_module=strip_tags($path_info[0] ?: $module_module);
            $this -> module =$has_child[$_SERVER['HTTP_HOST']].'\\' . $module_module;
        }
