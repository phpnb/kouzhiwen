    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if ($this->config['view_base']) {
                ...
                $path_count = explode(DS, $template);
                // 载入当前模块
                $module = isset($module) ? $module : Request::instance()->module();
                $module = str_replace('\\', '/', $module);

                // 载入当前主模块的其他子模块
                if (count($path_count) == 3) {
                    $tmp = explode('/', $module);
                    $module = $tmp[0] . '/' . $path_count[0];
                    unset($path_count[0]);
                    $template = implode('/', $path_count);
                }

                // 载入其他主模块
                if (count($path_count) == 4) {
                    $module = $path_count[0] . '/' . $path_count[1];
                    unset($path_count[0], $path_count[1]);
                    $template = implode('/', $path_count);
                }
