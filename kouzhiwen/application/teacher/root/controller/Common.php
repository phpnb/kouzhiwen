...
class Common extends Controller{
    ...
    public function _initialize(){
        ...
        // 通用权限
        $access['admin/root/Index'] = ['index','welcome','table','form','chart'];
    ...
    public function ExcelExport($title,$cellName,$data,$topNumber=1){
        ...
        foreach ($data as $k=>$v)
        {
            ...
            {
                if($v1[2] == 'created_at'){
                    $text = date('Y-m-d H:i:s',$v[$v1[2]]);
                }else if($v1[2] == 'data'){
                    $arr = json_decode($v['data'],true);
                    $da = json_decode($v['answer'],true);
                    $text = '';
                    foreach ($arr as $key => $value) {
                        ...
                        foreach ($value['answer'] as $key1 => $value1) {
                            # code...
                            $text .= '选项：'.$value1['id'].'.'.$value1['title']."\r\n";
                        }
