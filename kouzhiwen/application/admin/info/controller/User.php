...
class User extends Common{
    ...
    public function index(){
        ...
        $orderby = 'uid desc';
        // 获取数据
        $data  = admin_page($model, $where,$orderby);
    ...
    public function add(){
        if ($this -> isPost) {
            ...
            // 获取post数据
            $data = input('post.');
            $data['nickname'] = getRandomString(6);
    ...
    public function del(){
        ...
        $where['uid'] = ['in', $uid];
        $data['enterprise_id'] = $this->user['enterprise_id'];
//        dump($data['enterprise_id']);die;
        if($data['enterprise_id'] == 0){
            $model -> delData($where);
            return ajax('删除成功');

        }else{
            if (!$model -> editData($where,['updated_at'=>time(),'enterprise_id' => 0])) return ajax('删除失败');
            $this->AddLog("删除用户成功".$uid);
            return ajax('删除成功');
        }
    ...
    public function auditing(){
        $model = new model;
        if (!$this -> isPost) return ajax('非法请求');
        $allot=$this->param['allot']?$this->param['allot']:2;
        // 定义条件
        $uid = $this -> param['id'];
        if(empty($uid)){
            return ajax('没有必要参数');
        ...
        // 可批量删除
        $where['uid'] = ['in', $uid];
        $data=['allot'=>$allot,'updated_at'=>time()];
        $msg="审核不通过";
        $content="尊敬的用户你好：你所提交的个人信息没有通过后台审核；请重新修改信息修改。";
        if($allot==2){
            $msg="审核通过";
            $content="尊敬的用户你好：你所提交的个人信息以通过后台审核。";
            $data['enterprise_id']=$this->user['enterprise_id'];
        }
        if (!$model -> editData($where,$data)) return ajax('审核失败', 2);
        $UserNotice =new UserNotice();
        $data=array();
        $uidarr=explode(",",$uid);
        $time=time();
        foreach ($uidarr as $v){
            $data[]=array('uid'=>$v,'title'=>$msg,'content'=>$content,'reading'=>0,'created_at'=>$time);
        }
        $UserNotice->insertAll($data);
        $this->AddLog($msg."用户成功".$uid);
        return ajax('审核成功');
    }
    ...
    public function achievement(){
        // 搜索关键词
        $keywords = input('get.keywords');
        $this->param['datatype']=empty($this->param['datatype'])?'table':$this->param['datatype'];
        ...
        if($this->param['datatype']=='chart'){
            $data=$model->where($where)->limit('0,10')->select()->toArray();
            $arr=array('labels'=>array(),'score'=>array());
            if(!empty($data)){
                foreach ($data as $val){
                    $arr['labels'][]=$val['title'];

                    $arr['score'][]=$val['score']?$val['score']:0;
                }
            }
            $data=array('data'=>json_encode($arr),'page'=>1);
        }else{
            // 获取数据
    ...
    public function sign(){
        ...
        $field=array('s.id','c.name','c.photo','s.date','s.created_at');
        // 获取数据
...
//        $model = $UserRecord->getDataList(['type'=>4],$this->user['uid']);
     ...
     * 所报名学习班
     */
    public function classress(){
        // 搜索关键词
        $keywords = input('get.keywords');
        $where=array('uid'=>$this -> param['uid']);
        $user = new ClassUser();
        $eag = $user->where($where)->select()->toArray();
        $res = array_column($eag,'class_id');
        $classcourse = new Study();
        $data = $classcourse->whereIn('id',$res)->select()->toArray();
        $age['uid'] = $this -> param['uid'];
...
//         模板
    ...
    public function courseany(){
        ...
        $date = $ctype->where($where1)->select()->toArray();
        $res = array_column($date,'cid');
        $classcourse = new ClassCourse();
        $str = $classcourse->where('class_id',$this -> param['cid'])->select()->toArray();
        $data = $classcourse->where('class_id',$this -> param['cid'])->whereIn('id',$res)->select()->toArray();
...
//         模板
    ...
    public function import(){
        if(request()->isPost()){
            ...
            if(!empty($update)){
                $where['uid'] = ['in', $update];
                $res=$model->editData($where,['enterprise_id'=>$this->user['enterprise_id'],'allot'=>2,'updated_at'=>$time]);
